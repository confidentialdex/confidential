...
async function main() {
    ...
    for (const [pairName, pythId] of Object.entries(PAIR_PYTH_IDS)) {
        let maxLeverage;
        if (['BTC/USDC', 'ETH/USDC', 'SOL/USDC', 'EUR/USDC', 'GBP/USDC', 'USDJPY/USDC'].includes(pairName)) {
            maxLeverage = 100;
        } else if (['SPY/USDC', 'NVDA/USDC', 'AAPL/USDC', 'TSLA/USDC'].includes(pairName)) {
            maxLeverage = 20;
        } else {
            maxLeverage = 50;
        }

        const pairId = ethers.keccak256(ethers.toUtf8Bytes(pairName));
        console.log(`Setting leverage for ${pairName} to ${maxLeverage}x...`);
        try {
            const tx = await core.updatePairLeverage(pairId, maxLeverage);
            await tx.wait();
            console.log(`✅ ${pairName} leverage updated!`);
            await sleep(2000);
        } catch (e) {
            console.error(`❌ Failed to update leverage ${pairName}:`, e.message);
        }
    }
    
    console.log("All pair leverages configured!");
}
